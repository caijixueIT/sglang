...
class MooncakeKVManager(CommonKVManager):
    ...
    def send_kvcache_slice(
        ...
        src_heads_per_rank = self.kv_args.kv_head_num  # per-rank KV heads on prefill
        page_size = self.kv_args.page_size

        # Get total KV heads from kv_args (set from model config)
        # IMPORTANT: Must use the value from model config, fallback is only for compatibility
        if hasattr(self.kv_args, 'total_kv_heads'):
            total_kv_heads = self.kv_args.total_kv_heads
        else:
            # Fallback: try to infer from src_heads_per_rank
            # This works correctly only when prefill_tp <= total_kv_heads
            # When prefill_tp > total_kv_heads, src_heads_per_rank = 1 (replicated)
            # and we can't distinguish between (tp=8, kv=8) and (tp=16, kv=8)
            # Use a conservative estimate: assume no replication
        ...
        if prefill_tp <= total_kv_heads:
            # No replication on prefill: each rank has unique heads
            ...
            # Replication on prefill: multiple ranks share the same head
            # Only one rank per replicate group should send (rank % replicate == 0)
            if local_tp_rank_in_group % prefill_replicate != 0:
                return 0  # This rank doesn't send (another rank in the group will)
        ...
        if decode_tp <= total_kv_heads:
            # No replication on decode: each rank needs unique heads
            ...
            # Replication on decode: multiple ranks need the same head
        ...
        if overlap_start >= overlap_end:
            # No overlap: this prefill rank doesn't need to send to this decode rank
            return 0
        ...
        bytes_per_head_slice_to_send = dst_kv_item_len // page_size // dst_heads_per_rank
