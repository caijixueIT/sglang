...
class MoriKVManager(CommonKVManager):
    ...
    def _build_tp_slice_config(self, peer_info: KVArgsRegisterInfo) -> Optional[TPSliceConfig]:
        ...
        if prefill_tp <= total_kv_heads:
            ...
            # Only one rank per replicate group should send
    ...
    def send_kvcache(
        ...
        if self.is_mla_backend:
            ...
            if tp_mismatch:
                ...
                # tp_cfg is None when this prefill rank shouldn't send to this decode rank
                # (happens when decode TP > total KV heads and KV heads are replicated)
